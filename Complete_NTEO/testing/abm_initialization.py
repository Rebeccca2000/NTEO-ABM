...
class MobilityModelNTEO(Model):
    ...
    def debug_database_tables(self):
        ...
        try:
            ...
            if self.schema:
                # Check both tables using raw SQL
                commuter_count = session.execute(text("SELECT COUNT(*) FROM commuter_info_log")).fetchone()[0]
                booking_count = session.execute(text("SELECT COUNT(*) FROM service_booking_log")).fetchone()[0]
    ...
    def debug_booking_status_sql(self):
        ...
        try:
            ...
            if self.schema:
                # Raw SQL approach (when using schema)
                booking_status_query = text("""
                    SELECT 
                        status,
                        COUNT(*) as count,
                ...
                total_commuters_query = text("SELECT COUNT(*) FROM commuter_info_log")
                total_commuters = session.execute(total_commuters_query).fetchone()[0]
                
                commuters_with_bookings_query = text("""
                    SELECT COUNT(DISTINCT commuter_id) FROM service_booking_log
                """)
    ...
    def debug_commuter_activity_sql(self):
        ...
        try:
            ...
            if self.schema:
                # Check commuter distribution by characteristics
                commuter_breakdown_query = text("""
                    SELECT 
                        income_level,
                        COUNT(*) as total_commuters,
                        COUNT(DISTINCT sbl.commuter_id) as commuters_with_bookings
                    FROM commuter_info_log cil
                    LEFT JOIN service_booking_log sbl ON cil.commuter_id = sbl.commuter_id
